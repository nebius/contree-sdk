...
async def test_use_image_await_passes_tag_spec(fake_contree: Contree):
    ...
    process_state = ProcessState(
        continued=False, core_dump=False, exit_code=0,
        pid=1, signal=0, stopped=False, timed_out=False,
    )
    resources = ProcessResources(
        block_input=0, block_output=0, cost=0.0, elapsed_time=0.5,
        involuntary_switches=0, max_rss=1024, monotonic_time=0.5,
        page_faults=0, page_faults_io=0, shared_memory=0, signals=0,
        swaps=0, system_cpu_time=0.1, unshared_memory=0, user_cpu_time=0.1,
        voluntary_switches=0,
    )
    metadata = InstanceOperationMetadata(
        args=[], command="echo hello", cwd="/", disposable=True, env={},
        files={}, hostname="hostname", image="my-tag:latest", shell=True,
        stdin=StreamDescription(value="", encoding=StreamEncoding.ascii),
        timeout=60, truncate_output_at=65535,

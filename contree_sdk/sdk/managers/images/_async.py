...
class ImagesManager(ImagesBaseManager[ContreeImage]):
    ImageType = ContreeImage
    ...
    async def get_images_list(
        ...
        """
        return [image async for image in self.iter_images(number=number, tagged=tagged, since=since, until=until)]
    ...
    async def iter_images(
        ...
        now = datetime.now()
        async for image in self.client.api.iter_images(
            tagged=tagged,
    ...
    async def use_image(self, ref: str | UUID | OCIReference, strict: bool = False) -> ContreeImage:
        ...
        if isinstance(ref, UUID):
            if strict:
                return await self.get_image_by_uuid(ref)
    ...
    async def get_image_by_uuid(self, uuid: UUID | str) -> ContreeImage:
        if isinstance(uuid, str):
            uuid = UUID(uuid)
        return self.image_by_data(await self.client.api.inspect_image(str(uuid)))
    ...
    async def pull_image_by_oci(
        ...
        *,
        ...
        try:
            ...
            return await self.use_image(ref, strict=True)
        except NotFoundError:
            if isinstance(ref, UUID):
                raise
            logger.debug(f"Falling back to import: {ref}")
            return await self.import_image(ref, tag=tag, username=username, password=password, timeout=timeout)

    @keep_signature(get_images_list)
    async def __call__(self, *args, **kwargs) -> list[ContreeImage]:
        return await self.get_images_list(*args, **kwargs)

    async def __aiter__(self):
        async for image in self.iter_images():
            yield image

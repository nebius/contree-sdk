...
3. **`auth.ini`** — if the `contree` CLI is installed, credentials written by `contree auth login` are read from `~/.config/contree/auth.ini` (or `$CONTREE_HOME` / `$XDG_CONFIG_HOME/contree`).
